...
import { initScrollSync, toggleSyncScroll } from './scrollSync.js';
...
let sidebarData = loadFromLocalStorage(SIDEBAR_DATA_KEY) || {
  ...
  isLinkedScrolling: false // ✅ Explicitly set this to false
};
...
function initializeSidebar(sidebar) {
  ...
  // Create the fullscreen button list item
...
fullscreenIcon.className = 'bx bx-fullscreen'; // Use an appropriate icon
const fullscreenSpan = document.createElement('span');
fullscreenSpan.className = 'link_name';
fullscreenSpan.textContent = 'Fullscreen Mode';

fullscreenLink.title = 'Toggle fullscreen mode';

// Append elements
fullscreenLink.appendChild(fullscreenIcon);
fullscreenLink.appendChild(fullscreenSpan);
fullscreenLi.appendChild(fullscreenLink);

// Insert the fullscreen button into the sidebar
navLinks.appendChild(fullscreenLi);
...
// Update icon when fullscreen mode changes
...
const linkedScrollIcon = document.createElement('i');
...
// Append
linkedScrollLink.appendChild(linkedScrollIcon);
linkedScrollLink.appendChild(linkedScrollSpan);
linkedScrollLi.appendChild(linkedScrollLink);

// Insert this new Li before we insert toggleFramesOrientationLi
navLinks.appendChild(linkedScrollLi);

// Event listener for the linked scrolling toggle
linkedScrollLink.addEventListener('click', (e) => {
  e.preventDefault();
  
  // Ensure the variable is properly initialized
  if (typeof sidebarData.isLinkedScrolling === "undefined") {
    sidebarData.isLinkedScrolling = false;
    saveToLocalStorage(SIDEBAR_DATA_KEY, sidebarData); // ✅ Save reset state
  }

  // Toggle the state properly
  sidebarData.isLinkedScrolling = !sidebarData.isLinkedScrolling;

  // Update the icon properly
  linkedScrollIcon.className = sidebarData.isLinkedScrolling
      ? 'bx bx-toggle-right'
      : 'bx bx-toggle-left';

  // Apply explicit color styles
  linkedScrollIcon.style.color = sidebarData.isLinkedScrolling ? 'forestgreen' : 'white';
