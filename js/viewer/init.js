...
document.addEventListener('DOMContentLoaded', function () {
  ...
  urls.forEach((url, index) => {
    ...
    // Add click handler to track active frame
    containerFrame.addEventListener('click', () => {
      setActiveContainerFrame(containerFrame);
      setActiveFrame(containerFrame); // Update scroll sync active frame
    });
  ...
  initScrollSync(); // Initialize scroll sync functionality
  ...
  if (event.data && event.data.type === "iframeScrollDelta") {
    ...
    iframes.forEach((iframe) => {
      if (iframe.contentWindow !== event.source) {
        iframe.contentWindow.postMessage({ type: "syncScrollDelta", deltaPages: deltaPages }, "*");
        console.log("Parent: Forwarded syncScrollDelta to an iframe:", deltaPages);
      }
    });
  }
});